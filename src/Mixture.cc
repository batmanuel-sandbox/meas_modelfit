...
void evaluateDerivatives(
    ...
    evaluateDerivativesImpl(x, gradient, nullptr, false);
}
...
void evaluateDerivatives(
    ...
    evaluateDerivativesImpl(x.asEigen(), gradient.asEigen(), &hessian.asEigen());
}

void evaluateDerivativesImpl(
    eigen::Matrix<Scalar, eigen::Dynamic, 1> & x,
    eigen::Matrix<Scalar, eigen::Dynamic, 1> & gradient,
    eigen::Matrix<Scalar, eigen::Dynamic, eigen::Dynamic> * hessian,
    bool computeHessian
    ...
    gradient.setZero(gradient.size());

    if computeHessian {
        size_t rows = hessian.rows();
        size_t columns = hessian.columns();
        LSST_THROW_IF_NE(
            rows, _dim,
            pex::exceptions::LengthError,
